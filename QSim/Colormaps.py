"""
Contains functions for mapping scalar values to colors using color maps
"""
# Import modules
...
# Manually declare viridis colormap array from matplotlib.  That way we don't need to import the entire library
...
# Declare color mapping functions
def viridis(value: Union[float, ndarray], lower: float = 0.0, upper: float = 1.0):
    # Ensure value is a numpy array
    if isinstance(value, Number):
        value = array([value], dtype=float)

    # Ensure upper and lower aren't the same value
    if lower == upper:
        upper = lower + 1.0

    # Convert value array to RGB colors
    if len(value) > 0:
        # Remap to a 0-255 index
        mapped_value = (255 / (upper - lower) * (value - lower)).astype(int)
        # Truncate to the 0-255 range
        mapped_value[mapped_value < 0] = 0
        mapped_value[mapped_value > 255] = 255
    else:
        mapped_value = array([], dtype=int)

    # Convert to RGB
    colors = __viridis_map[mapped_value]
    colors = colors.astype('f')

    # Return the resulting color array
    return colors

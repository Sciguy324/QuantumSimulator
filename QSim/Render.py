...
from .Wavefunction import ContinuousState
...
class GLRenderer(pyglet.window.Window):
    ...
    def on_resize(self, width, height):
        ...
        self.program.uniforms['projection'].set(Mat4.orthogonal_projection(0, 1.0, 0, 5.0, -255, 255))
...
class GLRender1D(GLRenderer):
    """
    Renderer specifically for 1D continuous variable wavefunctions
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Vertex arrays
    ...
    def on_draw(self):
        super().on_draw()
        self.setStateVertexArray(self.simulation.state)
    ...
    def attachSimulation(self, simulation: Simulation):
        ...
        if simulation.state is None:
            raise ValueError('Simulation has no state set')

        # Check state dimension
        if simulation.state.dims != 1:
            raise ValueError(f"{simulation.state.dims}-dimensional system not compatible with a 1D viewer")

        # Call super-class function
        super().attachSimulation(simulation)

        # Build vertex list for the state vector
        self.setStateVertexArray(simulation.state)

    def setStateVertexArray(self, state: ContinuousState):
        x = state.basis[0]
        lines = np.zeros((len(x) * 2 - 2, 2), dtype=float)
        pts = np.array([x, state.toArray()]).T
        lines[::2] = pts[:-1]
        lines[1::2] = pts[1:]
